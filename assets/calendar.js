...
setInterval(() => countdownEls.forEach(({ el, target }) => updateOne(el, target)), 60000);
...
function buildExam(rows) {
  return rows.map(r => {
    ...
    return `<tr><td>${esc(r.name)}</td><td>${esc(r.admitCard)}</td><td>${esc(r.examDate)}</td><td>${cdEl}</td></tr>`;
  }).join('');
}
...
/* re-render bookmark state once user signs in / out, and show guest toast once */
...
document.addEventListener('authReady', async (e) => {
  ...
  if (!e.detail.user && !guestToastShown) {
